...
import p5 from "p5";
import { RECT2D , isRectCollideCenter , getRectCollisionSections } from './p5test.js'
...
const sketch = (p) => {
    ...
    p.draw = () => {
      ...
      for( let i=0; i<WORD_W; i++ ){
        let y = p.map( i , 0 , +WORD_W , 0 , container.clientWidth)
        if( i == WORD_W/2 )p.stroke( p.color(255,0,0) )
          else p.stroke( p.color(0,0,0) )
        p.strokeWeight(1);
        p.line(0,y,container.clientWidth,y)
        p.line(y,0,y,container.clientWidth)
      }
...
function createBox( t , x , y , z ){
  ...
  if( t == 0 ){
    ...
    box.setPosition( x , y , z * box.depth + box.depth/2 );
    return box;
  }
